...
- 연락처·계좌 탐지: 숫자를 한글·알파벳으로 바꿔 쓴 표기까지 인식해 사용자에게는 해당 부분에 밑줄과 안내로 알리고, 우회 흔적이나 사기 신고 이력이 있으면 관리자에게만 조용히 기록(같은 대상에 대한 사용자 신고와 자동 기록은 한 건으로 합침). 첫 대화 시작 메시지도 같은 검사를 거친다.
- 사기 이력 확인(`POST /api/chat/fraud-check`, 데모용 흉내 조회): 상대가 알려준 계좌·전화번호를 사용자가 직접 확인할 수 있다. 아무 번호나 조회하는 창구가 되지 않도록 **상대가 그 대화에서 보낸 번호만** 확인해 준다 — 내가 적어 넣은 번호는 조회되지 않고, 참여자가 아니면 거절한다. 옛 메시지도 끝까지 훑어 대조한다.
- 휴면 방 판정은 **먼저 나간 쪽** 기준이다. 한 사람이 나간 뒤 새 메시지가 오면 그 사람 목록에 방이 다시 떠 있으므로 지우지 않는다. 나갔던 사람이 다시 말을 걸면 나간 표시를 지운다.
