...
3. **채팅** — 상품 상세에서 "채팅하기" → `/chat/[id]`. 비속어 마스킹, 첫 답장 전 이미지 차단, 차단·신고.
4. **안전거래(에스크로)** — 상품 상세에서 "안전거래 요청"(금액 제안) → `/escrow/[id]`. 요청→조정→입금(보관)→수령확인(정산)/반환. 분쟁 접수 가능.
5. **관리자** — ADMIN 계정으로 `/admin`(대시보드)·`/admin/reports`(신고 처리·유저 정지)·`/admin/disputes`(분쟁 조정). 관리자 승격은 현재 DB 직접(운영 도구 범위): `docker compose exec -T db psql -U app -d app -c "UPDATE \"User\" SET role='ADMIN' WHERE nickname='<닉네임>'"`.
6. **회원 탈퇴** — `/mypage`. 진행 중인 에스크로·판매중 상품·최근 7일 판매완료가 있으면 차단된다.
...
- **채팅 실시간(WS):** 클라이언트가 아직 액세스 토큰을 보관하지 않아 브라우저에서 소켓을 열지 않는다 — 메시지 송수신은 REST로 온전히 동작(실시간 append는 후속). WS 서버 자체(인증·룸·마스킹 브로드캐스트)는 동작·테스트됨.
