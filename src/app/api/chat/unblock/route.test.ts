...
const unblockConversationCounterparty = vi.fn();
...
vi.mock("@/features/chat/service", async () => {
  ...
  return {
    ...actual,
    unblockConversationCounterparty: (...args: unknown[]) => unblockConversationCounterparty(...args),
  };
});
...
function req(body: unknown, cookie?: string): Request {
  return new Request("http://localhost/api/chat/unblock", {
    method: "POST",
...
beforeEach(() => {
  ...
  unblockConversationCounterparty.mockReset();
...
describe("POST /api/chat/unblock", () => {
  it("401s a guest and never calls the service", async () => {
    ...
    expect(unblockConversationCounterparty).not.toHaveBeenCalled();
  });
  ...
  it("200s and passes the AUTHENTICATED userId, never a body-supplied one — the target user id is derived server-side from conversationId", async () => {
    currentUserFromRefresh.mockResolvedValue({ userId: "real-blocker" });
    unblockConversationCounterparty.mockResolvedValue(undefined);
    const res = await POST(req({ conversationId: "c1", userId: "impersonated" }, `${REFRESH_COOKIE}=tok`));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true });
    expect(unblockConversationCounterparty).toHaveBeenCalledWith(getChatRepo(), "real-blocker", "c1");
  });

  it("maps a service AppError to its status", async () => {
    currentUserFromRefresh.mockResolvedValue({ userId: "u1" });
    unblockConversationCounterparty.mockRejectedValue(new AppError("NOT_FOUND", "대화를 찾을 수 없어요.", 404));
    const res = await POST(req({ conversationId: "c1" }, `${REFRESH_COOKIE}=tok`));
