...
vi.mock("@/features/chat/service", async () => {
  ...
  return { ...actual, unblockUser: (...args: unknown[]) => unblockUser(...args) };
});
...
describe("POST /api/chat/unblock", () => {
  ...
  it("200s and passes the AUTHENTICATED userId, never a body-supplied one", async () => {
    ...
    expect(unblockUser).toHaveBeenCalledWith(getChatRepo(), "real-blocker", "t1");
  });

  it("maps a service AppError to its status", async () => {
    currentUserFromRefresh.mockResolvedValue({ userId: "u1" });
    unblockUser.mockRejectedValue(new AppError("NOT_FOUND", "대상을 찾을 수 없어요.", 404));
    const res = await POST(req({ targetId: "t1" }, `${REFRESH_COOKIE}=tok`));
