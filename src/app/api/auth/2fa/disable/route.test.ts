...
vi.mock("@/features/auth/twofactor/service", () => ({
  disableTwoFactor: (...args: unknown[]) => disableTwoFactor(...args),
}));
...
describe("POST /api/auth/2fa/disable — step-up gating", () => {
  beforeEach(() => {
  ...
  it("succeeds and calls disableTwoFactor with the refresh-session userId when refresh auth and step-up agree", async () => {
    currentUserFromRefresh.mockResolvedValue({ userId: "u1" });
    ...
    expect(disableTwoFactor).toHaveBeenCalledWith(expect.anything(), "u1", expect.anything());
  });
