...
describe("registerUser", () => {
  ...
  it("maps a concurrent P2002 email collision (legacy string constraint mock, kept defensively) to 409 EMAIL_TAKEN", async () => {
    const create = vi.fn().mockRejectedValue({
      code: "P2002",
  ...
  it("maps a concurrent P2002 email collision (REAL @prisma/adapter-pg object shape { index }) to 409 EMAIL_TAKEN", async () => {
    const create = vi.fn().mockRejectedValue({
      code: "P2002",
      meta: { modelName: "User", driverAdapterError: { cause: { constraint: { index: "User_emailBlindIndex_key" } } } },
    });
    const db = fakeDb({ create });
    await expect(registerUser(db, input, noMeta)).rejects.toMatchObject({
      code: "EMAIL_TAKEN",
